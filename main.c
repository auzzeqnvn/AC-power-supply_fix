...
void    PROTECT(void)
{
    ...
    if(Uint_ADC_value > ADC_POSITIVE_6_SET_OVER)
    {
        Uc_Positive_6_Over++;
    ...
    if(Uint_ADC_value > ADC_NEGATIVE_6_SET_OVER)
    {
        Uc_Negative_6_Over++;
    ...
    if(Bit_Negative_6_Warning || Bit_Negative_12_Warning || Bit_Positive_12_Warning || Bit_Positive_6_Warning)
    {
        ...
        Uint_Warning_Count = 0;
    }
    else if(Uint_Warning_Count >= TIME_WARNING)
    {
        CONTROL_UNDER_24_ON;
    }
    ...
    else if(Uint_Warning_Count >= TIME_WARNING)
    {
     ...
     if(Uint_Warning_Count < TIME_WARNING)  
     {
         
        if(Uint_Warning_Count % TIME_BUZZER == 0)   CONTROL_BUZZER = !CONTROL_BUZZER;
        Uint_Warning_Count++;
     }
     else
     {
